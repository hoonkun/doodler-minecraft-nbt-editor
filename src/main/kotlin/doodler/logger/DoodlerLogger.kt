...
class DoodlerLogger {
    ...
    companion object {
        ...
        const val RECOMPOSITION_TAG = "RECOMPOSITION"

        val tagFilters = mutableSetOf(TEST_TAG, RECOMPOSITION_TAG)
        ...
        fun test(message: String) {
            if (!tagFilters.contains(TEST_TAG)) return

            log(TEST_TAG, message)
        }

        fun recomposition(from: String) {
            if (!tagFilters.contains(RECOMPOSITION_TAG)) return
